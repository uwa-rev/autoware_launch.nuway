...
def launch_setup(context, *args, **kwargs):
    ...
    cuda_pointcloud_preprocessor_node_param = ParameterFile(
        param_file=LaunchConfiguration("cuda_pointcloud_preprocessor_node_param_path").perform(context),
        allow_substs=True,
    )
...
>>>>>>> 4139483 (Working launcher)
    ...
    # For CUDA Preprocessing Node
    cropbox_parameters["cropbox.min_x"] = [vehicle_info["min_longitudinal_offset"], mirror_info["min_longitudinal_offset"]]
    cropbox_parameters["cropbox.max_x"] = [vehicle_info["max_longitudinal_offset"], mirror_info["max_longitudinal_offset"]]
    cropbox_parameters["cropbox.min_y"] = [vehicle_info["min_lateral_offset"], mirror_info["min_lateral_offset"]]
    cropbox_parameters["cropbox.max_y"] = [vehicle_info["max_lateral_offset"], mirror_info["max_lateral_offset"]]
    cropbox_parameters["cropbox.min_z"] = [vehicle_info["min_height_offset"], mirror_info["min_height_offset"]]
    cropbox_parameters["cropbox.max_z"] = [vehicle_info["max_height_offset"], mirror_info["max_height_offset"]]
    ...
    nodes.append(
        ComposableNode(
            ...
            parameters=[cuda_pointcloud_preprocessor_node_param]
        )
    )
...
def generate_launch_description():
    ...
    common_sensor_share_dir = get_package_share_directory("common_cuda_sensor_launch")
