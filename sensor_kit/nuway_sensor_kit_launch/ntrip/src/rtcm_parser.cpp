...
namespace ntrip_client
{
    ...
    RtcmParser::RtcmParser(
        rclcpp::Publisher<rtcm_msgs::msg::Message>::SharedPtr rtcm_pub, bool debug)
        : rtcm_pub_(rtcm_pub),
        ...
        while (!valid_messages_.empty())
        {
            if (debug_)
            {
                LogDebug("Publishing RTCM message with " +
                         std::to_string(valid_messages_.front().message.size()) + " bytes");
            }
    ...
    void RtcmParser::ProcessBuffer()
    {
        while (true)
        {
            ...
            if (ValidateCRC24Q(msg_start, total_length))
            {
                ...
                rtcm_msg.message.assign(msg_start, msg_start + total_length);
